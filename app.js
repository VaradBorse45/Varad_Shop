...
app.put('/update-order-status/:id', async (req, res) => {
  ...
  try {
    const order = await Order.findByIdAndUpdate(
      id,
      { status },
      { new: true } // This ensures that the updated order is returned after the update operation
    );
...
app.get('/getproductdetails/:shopId', async (req, res) => {
  try {
    const products = await Products.find({ shopId: req.params.shopId });
    res.json(products);
  } catch (error) {
